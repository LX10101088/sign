    ...
    var Table = {
        ...
        button: {
            ...
            batch : {
                ...
                title: __('batch'),
                extend: 'data-toggle="tooltip"',
                classname: 'btn btn-xs btn-success btn-batch'
            },
        },
        api: {
            ...
            bindevent: function (table) {
                ...
                //批量操作事件
                toolbar.on('click', Table.config.batchbtn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    var title = $(that).data('title') || $(that).attr("title") || __('batch');
                    var data = $(that).data() || {};
                    delete data.title;
                    //循环弹出多个编辑框

                    var url = options.extend.batch_url;
                    row = ids;
                    // row = $.extend({}, row ? row : {}, {ids: row[options.pk]});
                     url = url+'/ids/'+row;
                    Fast.api.open(url,typeof title === 'function' ? title.call(table, row) : title, data);
